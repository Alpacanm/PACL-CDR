...
class Model(nn.Module):
    ...
    def loss_graphcl(self, x1, x2, users, items):
        ...
        user_mask = (users >= 0) & (users < user_embeddings1.size(0))  
        valid_users = users[user_mask]

        item_mask = (items >= 0) & (items < item_embeddings1.size(0))  
        ...
        all_embs1_abs = all_embs1.norm(dim=1) + 1e-8 
        all_embs2_abs = all_embs2.norm(dim=1) + 1e-8  
