...
class Data(object):
    def __init__(self, opt, path):
        ...
        self.path = path
        self.n_batch = opt["n_batch"]
        self.batch_size = opt["batch_size"]
        self.train_num = opt["train_num"]
        self.sample_num = opt["sample_num"]

        try:
            train_file = self.path + '/coo_train.pkl'
            test_file = self.path + '/coo_test.pkl'
    ...
    def __next__(self):
        if self.current_batch_idx < self.n_batch:
            batch_data = self.mini_batch(self.current_batch_idx)
