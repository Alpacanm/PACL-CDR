...
class TaskGenerator(object):
    ...
    def read_train_data(self, train_file, ease):
        ...
        if self.opt["aggregator"] == "Transformer" and self.opt["task"] == "multi-item-intra":
            ease_dense = ease.to_dense()
        with codecs.open(train_file, "r", encoding="utf-8") as infile:
            ...
            for line in infile:
                ...
                if item not in positive_set[user]:
                    ...
                    elif self.opt["aggregator"] == "Transformer" and self.opt["task"] == "multi-item-intra":
                        positive_list[user].append([item, ease_dense[user][item]])
                    else:
        ...
        if self.opt["aggregator"] == "item_similarity" or (self.opt["aggregator"] == "Transformer" and self.opt["task"] == "multi-item-intra"): # extend items for 1-hop sampled graph_sage
            def takeSecond(elem):
                return -elem[1]
