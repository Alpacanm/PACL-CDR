...
class SUPCCL(GeneralRecommender):
    ...
    def randint_choice(high, size=1, replace=True, p=None, exclusion=None):
        ...
        if p is not None:
            raise NotImplementedError("'p' parameter is not supported.")

        if exclusion is not None and high <= len(exclusion):
            raise ValueError("The number of 'exclusion' is greater than or equal to 'high'.")

        exclusion_set = set(exclusion) if exclusion is not None else set()

        if not replace and (high - len(exclusion_set) < size):
            raise ValueError("There are not enough integers to sample without replacement.")

        result = []
        while len(result) < size:
            num = random.randint(0, high - 1)  # Random integer in range [0, high)
            if num not in exclusion_set:
                result.append(num)
                if not replace:
                    exclusion_set.add(num)
    ...
    def prepare_sub_graphs(self):
        ...
        for idx in tqdm(range(self.sub_graph_pool)):
            sub_mat_dict[idx] = self.get_norm_adj_mat(True, aug_type).to(
                self.device) if self.gpu_available else self.get_norm_adj_mat(True, aug_type)
        return sub_mat_dict
    ...
    def get_norm_adj_mat(self, is_subgraph=False, aug_type=0):
        ...
        if is_subgraph and aug_type in [0, 1, 2] and self.ssl_ratio > 0:
            ...
            if aug_type == 0:
                ...
                '''
                keep_idx = self.randint_choice(len(self.user_np), size=int(len(self.user_np) * (1 - self.ssl_ratio)),
                                          replace=False)
